...
@section('content')
    ...
    <div class="main">
        <div class="container">
            <h2 style="text-align: center !important;">@if(isset($fecha)){!!$fecha !!}@else <strong>Yesterday</strong> @endif</h2>
            <br>
            ...
            <div class="container" >
                <div  style="margin-left: 25.5%" class="btn-group" role="group" aria-label="...">
                            ...
                            <div class="widget-nopad">
                                <table class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td>Impressions</td>
                                            <td style="font-size: 15px !important;"><b>{{ number_format($total_impressions_month, 0, '', '.') }}</b></td>
                                        </tr>
                                        <tr>
                                            <td>Clikcs</td>
                                            <td style="font-size: 15px !important;"><strong>{{number_format($total_clicks_month,0,'','.')}}</strong></td>

                                        </tr>
                                        <tr>
                                            <td>CTR(%)</td>
                                            <td style="font-size: 15px !important;"> <strong>{{ number_format($total_ctr_month,5) }} %</strong></td>
                                        </tr>
                                        <tr>
                                            <td>eCPM</td>
                                            <td style="font-size: 15px !important;"><strong>$ {{ number_format($total_cpm_month,5) }}</strong></td>
                                        </tr>

                                        <tr>
                                            <td>Revenue Manager</td>
                                            <td style="font-size: 15px !important;" ><strong>${{ number_format($total_payment_month  ,2) }}</strong></td>
                                        </tr>

                                        <tr>
                                            <td>Revenue Plublishers</td>
                                            <td style="font-size: 15px !important;" ><strong>${{ number_format($total_payment_month * 0.05 ,2) }}</strong></td>
                                        </tr>
                                    </tbody>
        ...
        <div class="row">
            <div class="span12" style="margin-left: 50%">
                <a href="{{ url('admin/referrer') }}" class="btn btn-primary btn-lg">
                    <span class="fa fa-home"></span> Go Back</a>
            </div>
        </div>
        <br><br>
</div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">Information for the report</h4>
                </div>
                ...
                <div class="modal-footer">
                    <a id="search" type="button" class="btn btn-primary"> <span class="icon-bar-chart"></span> Search</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).on('ready', function () {
            ...
            @endif
               ...
               window.location.href=  BASEURL + 'admin/' + 'reportReferrer/' + 'yesterday/'+ '{{ $id }}';
           });
            @if(isset($fecha))
              @if($fecha == 'Eight Days')
          ...
          eightdays.on('click', function () {
                window.location.href=  BASEURL + 'admin/' + 'reportReferrer/' + 'eight/'+ '{{ $id }}';
            });
            ...
            @endif
             current.on('click', function () {
                window.location.href=  BASEURL + 'admin/' + 'reportReferrer/' + 'current/'+ '{{ $id }}';
            });
            @if(isset($fecha))
              @if($fecha == 'Last Month')
            ...
            last.on('click', function () {
                window.location.href=  BASEURL + 'admin/' + 'reportReferrer/' + 'last/'+ '{{ $id }}';
            });
            @if(isset($fecha))
            ...
            $('#search').on('click', function () {
                ...
                window.location.href=  BASEURL + 'admin/' + 'reportReferrer/' + 'custom/'+ '{{ $id }}/' + date + '/'+ toDate ;
            });
